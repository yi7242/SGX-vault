              ...
              sgx_ra_msg3_t **msg3, size_t &quote_size, std::string isv_url)
{
    ...
    *msg3 = (sgx_ra_msg3_t *)malloc(sizeof(sgx_ra_msg3_t) + quote_size);
    ...
    memcpy(msg3tmp->mac, base64_decode<uint8_t, char>((char *)res_json_obj["msg3"]["mac"].ToString().c_str(), tmpsz), 16);

    memcpy(msg3tmp->g_a.gx, base64_decode<uint8_t, char>((char *)res_json_obj["msg3"]["g_a"]["gx"].ToString().c_str(), tmpsz), 32);

    memcpy(msg3tmp->g_a.gy, base64_decode<uint8_t, char>((char *)res_json_obj["msg3"]["g_a"]["gy"].ToString().c_str(), tmpsz), 32);

    memcpy(msg3tmp->ps_sec_prop.sgx_ps_sec_prop_desc, base64_decode<uint8_t, char>((char *)res_json_obj["msg3"]["ps_sec_prop"]["sgx_ps_sec_prop_desc"].ToString().c_str(), tmpsz), 256);
                 ...
                 sp_config_t config, bool &attestation_accepted)
{
    ...
    if (get_attestation_report(ias, quote_b64, &msg4))
        return -1;
                              ...
                              std::string &ra_ctx_b64, uint8_t *&sk, uint8_t *&mk)
{
    ...
    if (!master_exist)
    {
        print_debug_message("Masterpass doesn't exist", INFO);
        print_debug_message("Creating Masterpass, input your pass:", INFO);

        std::string master;
        std::getline(std::cin, master);

        print_debug_message("Masterpass -> ", INFO);
        print_debug_message(master, INFO);

        uint8_t *master_send = (uint8_t *)master.c_str();
        size_t master_len = master.length() + 1;
        ...
        auto res = client.Post("/master_sealing", request_json, "application/json");
        response_json = res->body;
        res_json_obj = json::JSON::Load(response_json);
    ...
    {
        // 新たにパスワードを登録するか、以前登録したパスワードを検索するか選択
        print_debug_message("Do you want to register a new password or search for a previously registered password?", INFO);
        print_debug_message("1: Register a new password", INFO);
        print_debug_message("2: Search for a previously registered password", INFO);
        int choice;
        std::cin >> choice;
        std::cin.ignore();
        if (choice == 1)
        {
            print_debug_message("Input your masterpass", INFO);
            std::string master;
            std::getline(std::cin, master);
            print_debug_message("Name a label for your password", INFO);
            std::string label;
            std::getline(std::cin, label);
            print_debug_message("Input your password", INFO);
            std::string password;
            std::getline(std::cin, password);

            // 以下master, label, passwordを暗号化して送信
            uint8_t *plain_send_1 = (uint8_t *)master.c_str();
            uint8_t *plain_send_2 = (uint8_t *)label.c_str();
            uint8_t *plain_send_3 = (uint8_t *)password.c_str();

            size_t secret_1_len = master.length() + 1;
            size_t secret_2_len = label.length() + 1;
            size_t secret_3_len = password.length() + 1;

            uint8_t *iv_send = new uint8_t[12]();
            uint8_t *tag_send_1 = new uint8_t[16]();
            uint8_t *tag_send_2 = new uint8_t[16]();
            uint8_t *tag_send_3 = new uint8_t[16]();

            /* GCM方式は平文と暗号文の長さが同一 */
            uint8_t *cipher_send_1 = new uint8_t[secret_1_len]();
            uint8_t *cipher_send_2 = new uint8_t[secret_2_len]();
            uint8_t *cipher_send_3 = new uint8_t[secret_3_len]();
            ...
            if (-1 == (aes_128_gcm_encrypt(plain_send_3,
                                           secret_3_len, sk, iv_send, cipher_send_3, tag_send_3)))
            {
                return -1;
            }

            char *cs1_b64, *cs2_b64, *cs3_b64;
            char *ivs_b64;
            char *tags1_b64, *tags2_b64, *tags3_b64;
            ...
            cs3_b64 = base64_encode<char, uint8_t>(cipher_send_3, secret_3_len);
            ivs_b64 = base64_encode<char, uint8_t>(iv_send, 12);
            tags1_b64 = base64_encode<char, uint8_t>(tag_send_1, 16);
            tags2_b64 = base64_encode<char, uint8_t>(tag_send_2, 16);
            tags3_b64 = base64_encode<char, uint8_t>(tag_send_3, 16);
            ...
            req_json_obj["cipher3"] = cs3_b64;
            req_json_obj["iv"] = ivs_b64;
            req_json_obj["tag1"] = tags1_b64;
            req_json_obj["tag2"] = tags2_b64;
            req_json_obj["tag3"] = tags3_b64;
            ...
            /* 計算に使用する暗号データを送信 */
            auto res = client.Post("/add_password", request_json, "application/json");
            response_json = res->body;
            res_json_obj = json::JSON::Load(response_json);
            ...
            {
                print_debug_message("Masterpass is incorrect", ERROR);
                return -1;
            }
            else if (res->status != 200)
            {
        ...
        else if (choice == 2)
        {
            ...
            std::string master;
            std::getline(std::cin, master);
            print_debug_message("Input the label of the password you want to retrieve", INFO);
            std::string label;
            std::getline(std::cin, label);

            // master, label を暗号化して送信
            uint8_t *plain_send_1 = (uint8_t *)master.c_str();
            uint8_t *plain_send_2 = (uint8_t *)label.c_str();
            ...
            // パスワード取得リクエストを送信
            auto res = client.Post("/retrieve_password", request_json, "application/json");
            response_json = res->body;
            res_json_obj = json::JSON::Load(response_json);
            ...
            else if (res->status == 200)
            {
                ...
                if (-1 == (aes_128_gcm_decrypt(cipher_result,
                                               cipher_result_len, sk, iv_result, tag_result, plain_result)))
                {
                    print_debug_message("Failed to decrypt the password", ERROR);
                    return -1;
                }

                // 復号されたパスワードを出力
                print_debug_message("Retrieved password -> ", INFO);
                print_debug_message(std::string((char *)plain_result), INFO);
            }
            else
            {
                std::string message = "Unexpected error while processing password retrieval.";
                print_debug_message(message, ERROR);
                exit(1);
            }
        }
        ...
        {
            print_debug_message("Invalid input", ERROR);
            return -1;
        }
    }
    return 0;
